...
class TranscriptionPipeline:
    ...
    def _slice(self, wav: Path, out: Path, start: float, length: float) -> None:
        # Output-side seeking (-ss/-t after -i) plus a re-encode to clean PCM.
        # This is sample-accurate and always yields a valid WAV header, unlike
        # `-c copy` input seeking, which can produce chunks the decoder reads as
        # empty. The source is already 16 kHz mono PCM, so the re-encode is cheap.
        cmd = [
            "ffmpeg", "-y", "-loglevel", "error",
            "-i", str(wav),
            "-ss", str(start), "-t", str(length),
            "-ac", str(self.cfg.target_channels),
            "-ar", str(self.cfg.target_sample_rate),
            "-c:a", "pcm_s16le",
            str(out),
        ]
        proc = subprocess.run(cmd, capture_output=True, text=True)
